...
//By Zipeng.Feng
void LCD_Color_Fill(u16 x1, u16 y1, u16 x2, u16 y2, u16 *pColor)
{
    lv_color_t * color_p = (lv_color_t *)pColor;
    u16 x = x1, y = y1;
    u16 Color;

    for(y=y1;y<=y2;y++)
    {
        LCD_SetCursor(x1,y);   // éè??1a±ê???? ,x loaction never change
        LCD_WR_CMD(0X2C);			// ?aê?D′è?GRAM
        for(x=x1;x<=x2;x++)
        {
            Color = *(u16 *)color_p;
            *(__IO u16*)FSMC_DATA = Color;	//D′è?êy?Y 
            color_p++;
        }
    }
}
