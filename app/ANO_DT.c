...
ST_ANO_DT_FRAME_LIST_ITEM astAnoDtFrameListItem[] =
{
    {E_ANO_DT_FUNC_S2M_TYPE_STATUS, TRUE, TRUE, &stAnoDtDataS2MStatus, sizeof(ST_ANO_DT_DATA_S2M_STATUS), NULL, NULL},
    {E_ANO_DT_FUNC_S2M_TYPE_POWER, TRUE, TRUE,  &stAnoDtDataS2MPower, sizeof(ST_ANO_DT_DATA_S2M_POWER), NULL, NULL},
    {E_ANO_DT_FUNC_S2M_TYPE_RCDATA, TRUE, TRUE, &stAnoDtDataS2MRcdata, sizeof(ST_ANO_DT_DATA_S2M_RCDATA), NULL, NULL}
};
#endif
...
u8 ANO_DT_Send_Data(u8 *pu8Buff, u8 u8len)
{
    ...
    {
        u8Ret = USART3_SendData(pu8Buff, u8len);
    }
    return u8Ret;
}
...
void ANO_DT_Slaver_Send_Task(void * pvParameters)
{
    ...
    while(pdTRUE)
    {
        ...
        {
#endif
            xReturn = FR_OS_QueueReceive(hAnoDtSend_Queue, pstAnoDtSendData, ANO_DT_FRAME_PERIOD);
            if(xReturn == pdTRUE)
            {
