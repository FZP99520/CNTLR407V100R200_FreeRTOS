...
u8 bAdcInitDone = FALSE;
static u16 _au16JAdcValue[3] = {0};
static float _af32Voltage[3] = {0};

u16 au16AdcConvertVal[ADC_CHANNEL_INUSE_NUM] = {0};
u16 au16AdcConvertValOffset[ADC_CHANNEL_INUSE_NUM] = {0};
float af32AdcOutput[ADC_CHANNEL_INUSE_NUM] = {0, 0, 0, 0};

u8  _bADC1_OverRunFlag = FALSE;
ST_ADC_Conv_t stAdcConv =
{
    .u8ValLen = 4,
    .pu16AdcConvVal = au16AdcConvertVal,
    .pu16AdcConvValOffset = au16AdcConvertValOffset,
    .pf32AdcOutput = af32AdcOutput
};

TaskHandle_t hAdcAcquireTask;
...
void Adc_Acquire_Task(void *pvParameters)
{
    ...
    u8 u8TimesCnt = 0, u8temp = 0;
    float af32Sum[ADC_CHANNEL_INUSE_NUM] = {0, 0, 0};
    while(pdTRUE)
    {
        //set adc offset
        ...
        if(TRUE)
        {
            for(u8temp=0; u8temp<ADC_CHANNEL_INUSE_NUM; u8temp++)
            {
                af32Sum[u8temp] += au16AdcConvertVal[u8temp];
            }

            u8TimesCnt++;
            if(u8TimesCnt == ADC_GET_OFFSET_NUM)
            {
                for(u8temp=0; u8temp<ADC_CHANNEL_INUSE_NUM; u8temp++)
                {
                    au16AdcConvertValOffset[u8temp] = af32Sum[u8temp]/ADC_GET_OFFSET_NUM;
                }
...
void Adc_Init(void)
{
    ...
    DMA_ADC1_Init((u32)au16AdcConvertVal);
...
void ADC_Data_Handle(void)
{
    ...
    af32AdcOutput[3] = -((float)au16AdcConvertVal[3] - au16AdcConvertValOffset[3])*3.3f/4095;

    stAnoDtDataS2MStatus.s16YAW = af32AdcOutput[1]*10000;
    stAnoDtDataS2MStatus.s16PIT = af32AdcOutput[2]*10000;
    stAnoDtDataS2MStatus.s16ROL = af32AdcOutput[3]*10000;
...
{
    return stAdcConv.pf32AdcOutput;
}
...
void ADC_IRQHandler(void)
{
    ...
    {
        DMA_ADC1_Init(au16AdcConvertVal);
